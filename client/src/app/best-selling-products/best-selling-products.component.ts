...
export class BestSellingProductsComponent {
  ...
  loadBestsellerProducts() {
    this.loading = true;
    this._api.getBestsellerProducts().subscribe({
      next: (res) => {
        console.log("Fetched Bestseller Products:", res);
        this.bestsellerProducts = res;
        this.loading = false;
        setTimeout(() => this.initializeSwiper(), 500);
      },
      error: (err) => {
  ...
  initializeSwiper() {
    new Swiper('.bestseller-carousel', {
      modules: [Navigation, Pagination],
      slidesPerView: 4,
      spaceBetween: 15,
      navigation: {
        nextEl: '.bestseller-carousel-next',
        prevEl: '.bestseller-carousel-prev',
      },
      breakpoints: {
        320: { slidesPerView: 1 },
        768: { slidesPerView: 2 },
