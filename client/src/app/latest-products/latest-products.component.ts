...
export class LatestProductsComponent {
  ...
  loadLatestProducts() {
    this.loading = true;
    this._api.getLatestProducts().subscribe({
      next: (res) => {
        console.log("Fetched Latest Products:", res);
  ...
  initializeSwiper() {
    new Swiper('.latest-carousel', {
      modules: [Navigation, Pagination],
      slidesPerView: 4,
      spaceBetween: 15,
      navigation: {
        nextEl: '.latest-carousel-next',
        prevEl: '.latest-carousel-prev',
      },
      breakpoints: {
        320: { slidesPerView: 1 },
        768: { slidesPerView: 2 },
